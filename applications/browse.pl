...
graph_table(Graphs, Options) -->
	...
	html(
	  form([action(Action),class('graph-table')], [
	    \bs_table(
	      \html_table_header_row(['RDF Graph','Triples','Persistency']),
	...
	reply_html_page(cliopatria(default),
	  ...
	  [
	    ...
	    div(class=row, [
	      ...
	      div(class='col-xs-4',
	        \simple_search_form([id(ac_find_in_graph),filter(graph(Graph)),label('Search this graph')])
	      )
	    ]),
...
graph_persistency(Graph) -->
	...
	html([
	  ...
	  \bs_table(
	    \html_table_header_row(['Role','File','Size','Modified']),
...
graph_journal(Graph) -->
	...
	html(tr([td('Journal'),\file_info(File),\flush_journal_button(Graph)])).
graph_journal(_) --> [].
...
class_table(Pairs, Graph, Options) -->
	...
	html_requires(css('rdf.css')),
	bs_table(
	  \html_table_header_row(['Class','#Instances']),
...
instance_table(Pairs, Options) -->
	...
	html_requires(css('rdf.css')),
	bs_table(
	  \html_table_header_row(['Instance','#Properties']),
...
spo_header(P) -->
	{ nonvar(P) },
	html_table_header_row(['Subject','Object']).
spo_header(_) -->
...
sp_header(_) -->
	html_table_header_row(['Subject','Predicate']).
...
ns_table(html, Pairs) -->
	bs_table(
	  \html_table_header_row(['Prefix','URI']),
